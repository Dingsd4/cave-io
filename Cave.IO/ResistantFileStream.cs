...
namespace Cave.IO
{
    ...
    public class ResistantFileStream : Stream
    {
        ...
        public ResistantFileStream(string filename, FileMode mode = FileMode.Open, FileAccess access = FileAccess.Read, FileShare share = FileShare.Read,
        ...
        public static ResistantFileStream OpenSequentialRead(string filename) => new ResistantFileStream(filename);
...
#pragma warning restore CA1031
        ...
        ///     number of bytes are not currently available, or zero if the end of the stream is reached.
        /// </returns>
        public override int Read(byte[] buffer, int offset, int count) { return Resistant(() => { return stream.Read(buffer, offset, count); }); }

        /// <summary>Sets the current position of this stream to the given value.</summary>
        /// <param name="offset">The point relative to origin from which to begin seeking.</param>
        /// <param name="origin">
        ///     Specifies the beginning, the end, or the current position as a reference point for offset, using a
        ///     value of type SeekOrigin.
        /// </param>
        /// <returns>The new position in the stream.</returns>
        public override long Seek(long offset, SeekOrigin origin) { return Resistant(() => { return stream.Seek(offset, origin); }); }
